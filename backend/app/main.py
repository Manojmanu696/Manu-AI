...
app = FastAPI(title="Manu AI API", version="0.4.0", description="Local-first personal discovery dashboard API", lifespan=lifespan)
...
def chat(payload:ChatRequest,db:Session=Depends(get_db)): return current_provider(db).respond(payload.message,retrieve_context(db,payload.message))
