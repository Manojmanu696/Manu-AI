...
app = FastAPI(title="Manu AI API", version="0.4.1", description="Local-first personal discovery dashboard API", lifespan=lifespan)
...
def import_memories(payload:MemoryImportRequest,db:Session=Depends(get_db)):
    ...
    seen=set(); imported=[]
    categories={"movie":"media_taste","anime":"media_taste","game":"games","book":"books","project":"software_engineering","python":"software_engineering","preference":"preferences","prefer":"preferences","like":"preferences","watchlist":"watchlist"}
    for line in chunks:
        key=line.lower()
        if key in seen: continue
        seen.add(key); category=next((v for k,v in categories.items() if k in key),"imported_context")
        m=Memory(category=category,content=line,source="chatgpt_import",confidence=1.0,is_demo=False)
        db.add(m); imported.append(m)
    db.commit()
...
def memory_snapshot(db:Session=Depends(get_db)):
    ...
    explicit=[serialize_memory(m) for m in memories if m.source in ("user","chatgpt_import")]
    inferred=[serialize_memory(m) for m in memories if m.source=="inferred"]
...
def chat(payload:ChatRequest,db:Session=Depends(get_db)):
    """Answer with Ollama when available, but never leave the UI with an unhandled 500."""
    message=payload.message.strip()
    if not message: raise HTTPException(400,"message is required")
    context=retrieve_context(db,message)
    provider=current_provider(db)
    try:
        response=provider.respond(message,context)
        if response.get("mode") == "ollama":
            response["mode"] = "local"
            response["provider"] = "ollama"
        return response
    except OllamaUnavailable as exc:
        # Ollama can disappear while the app is running (model unloaded, service stopped, timeout, etc.).
        # Fall back to the deterministic local engine so the chat surface still works.
        from .services.ai import LocalProvider
        fallback=LocalProvider(db).respond(message,context)
        fallback["mode"]="local-fallback"
        fallback["provider"]="recommendation-engine"
        fallback["assumptions"]=(fallback.get("assumptions") or []) + [f"Ollama unavailable: {str(exc)[:180]}"]
        return fallback
