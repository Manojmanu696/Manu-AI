...
import { API_URL, api, MediaItem } from "@/lib/api";
...
export default function HomeV2() {
  ...
  async function runSearch(e?: FormEvent) {
    ...
    try {
      const search = await fetch(`/api/search?q=${encodeURIComponent(clean)}&category=${category}`);
      ...
      if (mode === "ai") {
        ...
        const ai = await api<ChatResponse>("/chat", {
          method: "POST",
      ...
      <Sidebar
        ...
        onNavigate={(id) => {
          if (id === "home") setView("home");
            ...
            setView("library");
          } else if (id === "discover") setView("home");
          else setView("home");
        }}
      />
              ...
              <section className={styles.searchZone}>
                <div className={styles.modeRow}>
                  <div className={styles.modeSwitch} aria-label="Search mode">
                    <button className={mode === "ai" ? styles.modeActive : ""} onClick={startAI}><span>✦</span> AI Search</button>
                    <button className={mode === "web" ? styles.modeActive : ""} onClick={openWeb}><span>⌕</span> Web Search</button>
                  </div>
                ...
                <form className={styles.searchBox} onSubmit={runSearch}>
                  ...
                  <input
                    ...
                    placeholder={mode === "ai" ? "Ask anything — or search for a movie, video, website…" : "Search the web for anything…"}
                    autoComplete="off"
                  />
                  {query && <button type="button" className={styles.clear} onClick={() => { setQuery(""); setResults([]); setAnswer(null); }}>×</button>}
                  <button className={styles.searchButton} disabled={loading}>{loading ? "Searching…" : "Search"}</button>
                </form>

                <div className={styles.categories}>
                  {categories.map((c) => <button key={c.id} className={category === c.id ? styles.categoryActive : ""} onClick={() => selectCategory(c.id)}><Icon name={c.icon} size={15} /> {c.label}</button>)}
                </div>

                <div className={styles.helperRow}>
                  <span>⌘ K to focus</span>
                  <span>Live public-web results</span>
                  <span>Local taste stays local</span>
                </div>
              </section>
                ...
                <section className={styles.resultsLayout}>
                  ...
                  <aside className={styles.sideColumn}>
                    <div className={styles.sideCard}>
                      ...
                      <button onClick={() => { setMode("web"); setCategory("videos"); runSearch(); }}>Find videos</button>
                      <button onClick={() => { setMode("web"); setCategory("movies"); runSearch(); }}>Find movies</button>
                      <button onClick={() => { setMode("web"); setCategory("games"); runSearch(); }}>Find games</button>
                      <button onClick={() => { setMode("web"); setCategory("websites"); runSearch(); }}>Find websites</button>
                    </div>
                    <div className={styles.sideCard}>
                      <span className={styles.cardLabel}>SEARCH MODE</span>
                      <p>{mode === "ai" ? "AI mode searches the public web first, then asks Manu AI to interpret the results." : "Web mode gives you direct search results without an AI rewrite."}</p>
                    </div>
                  </aside>
                </section>
              )}

              {view === "home" && <HomeSuggestions onSearch={(text, cat) => { setQuery(text); setCategory(cat); requestAnimationFrame(() => runSearch()); }} />}
            </div>
          )}
        </div>
      </section>
...
function SearchResultCard({ result, category }: { result: SearchResult; category: Category }) {
  ...
  return <a className={styles.resultCard} href={result.url} target="_blank" rel="noreferrer">
    <div className={styles.resultIcon}>{isVideo ? "▶" : "↗"}</div>
    <div className={styles.resultBody}>
      <div className={styles.resultUrl}>{host}</div>
      <h3>{result.title}</h3>
      <p>{result.snippet || "Open this result to read the full page."}</p>
    </div>
...
function HomeSuggestions({ onSearch }: { onSearch: (q: string, category: Category) => void }) {
  ...
  return <section className={styles.discovery}>
    <div className={styles.discoveryHead}><div><span>START HERE</span><h2>What do you want to find?</h2></div><p>Pick a category or type anything. Manu will search instead of sending you to another page.</p></div>
    <div className={styles.suggestionGrid}>{suggestions.map(([text, cat]) => <button key={text} onClick={() => onSearch(text, cat)}><small>{cat.toUpperCase()}</small><strong>{text}</strong><span>Search →</span></button>)}</div>
  </section>;
}
...
function LibraryView({ category, items, loading, onSearch }: { category: Category; items: MediaItem[]; loading: boolean; onSearch: (q: string) => void }) {
  ...
  return <div className={styles.libraryPage}>
    <div className={styles.libraryHero}><span>YOUR LIBRARY</span><h1>{category === "all" ? "Everything" : categories.find(c => c.id === category)?.label}</h1><p>{visible.length} saved locally. Search the public web below when your library is empty.</p></div>
    <div className={styles.librarySearch}><Icon name="search"/><input placeholder={`Search ${category === "all" ? "your library" : "your " + category}…`} onChange={e => onSearch(e.target.value)} /></div>
    {loading ? <div className={styles.noResults}>Loading your local library…</div> : visible.length ? <div className={styles.libraryGrid}>{visible.map(item => <article className={styles.libraryCard} key={item.id}><div className={styles.libraryType}>{item.media_type}</div><h3>{item.title}</h3><p>{item.genres || item.description || "No description yet."}</p>{item.personal_rating ? <b>★ {item.personal_rating}/10</b> : <small>{item.status.replace("_", " ")}</small>}</article>)}</div> : <div className={styles.emptyLibrary}><div>✦</div><h2>No saved {category === "all" ? "items" : category} yet.</h2><p>This is no longer a dead end. Use the search box above to discover current {category === "all" ? "movies, games, books, videos and websites" : category} from the web.</p></div>}
  </div>;
}
